...
def lorentzian_1d(x, *p):
    ...
    return A * sigma**2 / (x_shift**2 + sigma**2)
...
def gaussian_2dr(x, *p):
    ...
    xr = xc * np.cos(param[6]) - yc * np.sin(param[6])
    yr = xc * np.sin(param[6]) + yc * np.cos(param[6])
